...
@section('content')
    ...
    @can('task_create')
    <p>
        <a href="{{ route('tasks.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
    ...
    <ul class="list-inline">
        <li><a href="{{ route('tasks.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li> |
        <li><a href="{{ route('tasks.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('quickadmin.qa_trash')</a></li>
    </ul>
    </p>
    @endcan
...
@section('javascript')
    <script>
        @can('task_delete')
            @if ( request('show_deleted') != 1 ) window.route_mass_crud_entries_destroy = '{{ route('tasks.mass_destroy') }}'; @endif
        @endcan
